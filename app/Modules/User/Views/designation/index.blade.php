@extends('main_layouts.app')

@section('content')
<div class="row animated zoomIn">
	@include('errorOrSuccess')
	<div class="col-md-12">
		@include('User::designation.filter')
	</div>
	<div class="col-md-12">
		<!-- BEGIN EXAMPLE TABLE PORTLET-->
		<div class="portlet light bordered">
			<div class="portlet-title">
				<div class="caption font-green">
					<!-- <i class="icon-settings font-green"></i> -->
					<span class="caption-subject bold uppercase">Designation</span>

				</div>
				<button data-toggle="modal" data-target="#add-designation-modal" type="button" class="btn btn-success bnt-lg pull-right"><i class="fa fa-plus"></i> Add</button>
			</div>
			<div class="portlet-body">
				<table class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							<th style="padding: 8px !important;">ক্রমিক নং</th>
							<th style="padding: 8px !important;">Title</th>
							<th style="padding: 8px !important;">Status</th>
							<th style="padding: 8px !important;">Action</th>
						</tr>
					</thead>
					<tbody>
					@if(count($designations) > 0)
						@php $i = $designations->toArray()['from'];  @endphp
						
						@foreach($designations as $designation)
						<tr>
							<td>{{ en2bn($i++) }}</td>
							<td>{{$designation->title}}</td>
							<td>
								@if($designation->status == 0)
								Inactive
								@elseif($designation->status == 1)
								Active
								@else
								N/A
								@endif
							</td>
							<td class="action_buttons_style">
								<button  data-toggle="modal" data-target="#edit-designation-modal-{{$designation->id}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> Edit</button>
								@include('User::designation.edit')
								<form action="{{ route('designation.destroy',$designation->id) }}" method="POST">
									{{ method_field('DELETE') }}
									{{ csrf_field() }}
									<button class="btn btn-danger btn-xs" onclick="return confirm('Are you sure to delete this Designation')" title="Delete"><i class="fa fa-trash-o"></i>Delete </button>
				 				</form>
							</td>
						</tr>
						@endforeach
						@endif
					</tbody>
				</table>
			</div>
			@if(count($designations)>0)
				{{$designations->appends($_REQUEST)->render()}}
			@endif
		</div>
		<!-- END EXAMPLE TABLE PORTLET-->
	</div>

</div>
@include('User::designation.create')
<script type="text/javascript">
	$(document).ready(function () {

		highlight_nav('designation');
	});
</script>

@endsection