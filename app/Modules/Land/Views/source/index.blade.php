				...
				<table class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							...
							<th style="padding: 8px !important;">Source title</th>
					...
					<tbody>
						@if(count($sources) > 0)
						@php $i = $sources->toArray()['from'];  @endphp
						...
						<tr>
							...
							<td>
								<button  data-toggle="modal" data-target="#edit-land_source-modal-{{$source->id}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> Edit</button>
								@include('Land::source.edit')
								<form action="{{ route('source.destroy',$source->id) }}" method="POST">
									{{ method_field('DELETE') }}
									{{ csrf_field() }}
									<button class="btn btn-danger btn-xs" onclick="return confirm('Are you sure to delete this source')" title="Delete"><i class="fa fa-trash-o"></i>Delete </button>
								</form>
							</td>
						</tr>
						@endforeach
