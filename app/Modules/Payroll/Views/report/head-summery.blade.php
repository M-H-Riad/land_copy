<html>
<head>
    <title>Monthly Salary Report : {{$monthData->title}} {{$departmentGroup->group_name}}</title>
...
{{--                @php  $netPayable = $totalGross - $totalDeduction @endphp--}}
                <td class="right">{{numberWithZero($data->net_payable)}}</td>
            </tr>
        </table>
        <?php
        $poisaNumber = 0;
        $poisa = explode('.',number($data->net_payable));
        if(isset($poisa[1])){
            $poisaNumber = $poisa[1];
        }
        ?>
        <span style="font-size: 11px">In Word:<br/> {{convert_number(($data->net_payable)) .' taka and '.convert_number($poisaNumber)}} Poisha</span>
    </div>
@endif