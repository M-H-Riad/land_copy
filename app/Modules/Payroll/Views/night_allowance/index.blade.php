            ...
            <div class="portlet light bordered">
                <div class="portlet-title row">
                    <div class="caption font-green col-md-10">
                        ...
                        <span class="caption-subject bold uppercase">Night Allowance</span>
                    </div>

                    <div class=" actions col-md-2 side-padding-none">

{{--                        {!! Form::open(['route'=>'night_allowance.demo','method' => 'post'] ) !!}--}}
...
{{--                        </div>--}}
            ...
            <div class="portlet-body">
                ...
                <table class="table table-striped table-bordered table-hover" id="nopagination">
                    ...
                    <tbody>
                    @php $i = $nightAllowances->toArray()['from'] @endphp
                    @foreach($nightAllowances as $indexKey => $row)
                        <tr>
                            <td>{{$i++}}</td>
                            <td>{{ $row->title}}</td>
                             <td>
                                <a class="btn btn-primary btn-sm " data-style="zoom-in" href="{{ route('night_allowance.show',$row->id) }}">
                ...
                @if(count($nightAllowances)>0)
                    {{$nightAllowances->appends($_REQUEST)->render()}}
                @endif
            </div>
        </div>
    ...
    <div id="add-night-allowance-month" class="modal fade" role="dialog" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                {!! Form::open(['route' => 'night_allowance.store', 'method' => 'post','files'=>True]) !!}
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                    <h4 class="modal-title">Add Night Allowance</h4>
                </div>
