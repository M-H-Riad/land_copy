...
@if(isset($employee))
    ...
    <table style="margin-bottom: 10px">
        ...
        <tr>
            ...
            <th style="border: none!important; text-align: left;width: 20%"><b>PFNO : {{ $employee->pfno }} </b></th>
            <th style="border: none!important; text-align: left;width: 25%"><b>Designation : {{ $employee->designation->title }}</b></th>
            <th style="border: none!important; text-align: left;width: 25%"><b>Department : {{ $employee->department->department_name }}</b></th>
        </tr>
    </table>
            ...
            <tr>
                <td>
                    ...
                    @if(isset($empData->name))
                        <b>{{$value->title}}</b> <br/>
                        <span class="light">{{$empData->bank_acc}}, {{$empData->grade}} <br/></span>
    ...
    @foreach($overTimeReports as $overtime)
       @if($overtime->overTimeEmployee !=null)
        <tr>
            ...
            <td style="text-align: center;width: 10%">{{  $overtime->overTimeEmployee->type == "Single" ? $overtime->overTimeEmployee->hours : 0 }}  </td>
            <td style="text-align: center;width: 10%">{{  $overtime->overTimeEmployee->type == "Double" ? $overtime->overTimeEmployee->hours : 0 }}  </td>
...
@if(isset($nightAllowanceReport))
    <table  style="margin-bottom: 10px">
        ...
        <tr>
            ...
            <th colspan="5"><b>Night Allowance Report</b></th>
        </tr>
        <tr>
        ...
        @foreach($nightAllowanceReport as $nightAllowance)
            @if($nightAllowance->nightAllowanceEmployee !=null)
                <tr>
                    ...
                    <td style="text-align: center;width: 20%">{{ $nightAllowance->nightAllowanceEmployee->nights }} </td>
...
@endif
@php    $totalIfterBill = 0;  @endphp
@if(isset($ifterBillReport))
    <table  style="margin-bottom: 10px">
        <thead>
        <tr>

            <th colspan="6"><b>Ifter Bill Report</b></th>
        </tr>
        <tr>
        ...
        @php
            $rStamp = 0 ;
        @endphp
        @foreach($ifterBillReport as $ifterBill)
            @if($ifterBill->ifterBillEmployee !=null)
                <tr>
                    <td style="text-align: center;width: 20%"><b>{{ $ifterBill->title }}</b> </td>
                    <td style="text-align: center;width: 20%">{{ $ifterBill->ifterBillEmployee->per_day }} </td>
                    <td style="text-align: center;width: 20%">{{ $ifterBill->ifterBillEmployee->days }} </td>
                    <td style="text-align: center;width: 20%"> 10  </td>
                    <td style="text-align: center;width: 20%">{{ $ifterBill->ifterBillEmployee->ifter_bill }}  </td>
                </tr>
                @php
                    $rStamp += 10;
                    $totalIfterBill += $ifterBill->ifterBillEmployee->ifter_bill ;
                @endphp
            @endif
        @endforeach
        </tbody>
...
@php
     $grandTotal =   $total['net_payable'] + $totalIfterBill + $totalNightAllowance + $totalOvertime;
@endphp
