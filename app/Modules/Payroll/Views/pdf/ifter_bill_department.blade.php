...
<?php $i = 1 ;  $void_date =   request('date') != null ?  Date('d M,Y', strtotime(request('date')))  :  Date('d M,Y', strtotime("+3 days"))  ?>
    ...
    @if($ifterBillEmployee->count() > 0)
        @php $total = 0 ; @endphp
        @foreach($ifterBillEmployee as $employee)
            @php
        ...
        ?>
            <div style="font-size: 13px;">
                ...
                <p>We are pleased to authorize your good bank to make transfer of Tk.=<strong>{{$total}}</strong>   ({{convert_number(($total)) .' Taka and '.convert_number($poisaNumber)}} Poisha)
                    from <strong>Account Number {{$ifterBillDepartment->bank_account_number}}</strong>, Ifter Bill to the <strong> {{$ifterBillEmployee->count()}} employees</strong> of DWASA as per attached statement.
                    This is Ifter Bill of {{ $ifterBillDepartment->title }} Dhaka WASA for the month of <strong>{{$title}}</strong>.</p>
                ...
                <p>Department name: {{ $ifterBillDepartment->title }}</p>
        ...
        <table>
            ...
            @foreach($ifterBillEmployee as $employee)
                ...
                <tr>
                    ...
                    <td style="width: 20%!important;">{{  $info->first_name . ' ' . $info->middle_name . ' ' . $info->last_name  }}</td>
{{--                    <td style="width: 20%!important;">{{$employee->designation->title}} </td>--}}
