...
@if(isset($results))
    ...
    @foreach($results as $row)
        @php
            $i++;
           $total += $row->net_payable > 0 ? $row->net_payable : $row->bonus ;
        @endphp
    ...
    <div style="font-size: 13px;">
        ...
        <p>We are pleased to authorize your good bank to make transfer of Tk.=<strong>{{number($total)}}</strong> ({{convert_number(($total)) .' Taka and '.convert_number($poisaNumber)}} Poisha)
            ...
            This is bonus of {{ $departmentGroup->group_name }} Division for the month of <strong>{{$monthData->title}}</strong>.</p>
