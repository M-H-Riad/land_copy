...
@foreach($departments as $department)
    @if($department->nightAllowanceEmployee->count())
        @php $total = 0 ; @endphp
        @foreach($department->nightAllowanceEmployee as $employee)
            @php
        ...
        @if($department_id && $i=1)
            <div style="font-size: 13px;">
                ...
                <p>We are pleased to authorize your good bank to make transfer of Tk.=<strong> {{$total}}</strong>  ({{convert_number(($total)) .' Taka and '.convert_number($poisaNumber)}} Poisha)
                    from<strong>  Account Number {{$nightAllowanceDepartment->bank_account_number}}</strong> , OT to the<strong>  {{$department->nightAllowanceEmployee->count()}} employees </strong> of DWASA as per attached statement.
                    This is Night Allowance Bill of {{ $department->department_name }} Dhaka WASA for the month of <strong>  {{$title}}</strong> .</p>
