<div id="permanent-address-modal" class="modal fade" role="dialog" aria-hidden="true">
      ...
      <div class="modal-body">
        ...
        <table class="table table-bordered table-hover dt-responsive" class="nopagination" width="100%">
          ...
          <tr>
            ...
            <td>{{ Form::select('division_id',$division,$address->division_id, ['class' => 'form-control division_id','style' => 'width:100%','id' => 'division_id_1','onchange' => 'get_district_edit(1,'. $address->district_id .')']) }}</td>
          </tr>
          <tr>
            <th width="50%">Name Of District</th>
            <td>{{ Form::select('district_id',[],$address->district_id, ['class' => 'form-control district_id' , 'style' => 'width:100%','id' => 'district_id_1','onchange' => 'get_thana_edit(1,'. $address->thana_id .')']) }}</td>
          </tr>
          <tr>
            <th width="50%">Thana/Upazila</th>
            <td>{{ Form::select('thana_id',[],$address->thana_id, ['class' => 'form-control thana_id' , 'style' => 'width:100%','id' => 'thana_id_1','onchange' => 'get_post_office_edit(1,'. $address->post_office .')']) }}</td>
          </tr>
          <tr>
              <th width="50%">Post Office</th>
              <td>{{ Form::select("post_office",[],$address->post_office,["class" => "form-control focus_it",'id'=>'post_office_id_1']) }}</td>
            </tr>
          <tr>
